...
import AnimatedGradient from "../../assets/images/animatedGradient.png";
import React, { useEffect, useRef, useState, useCallback, memo } from "react";
...
const AnimatedGradientsHeroSection: React.FC = memo(() => {
  ...
  // Use your exact same throttling approach
  const FRAME_RATE = 1000 / 24; // 24fps like your original
  ...
  // Replicate your exact update pattern but for PNG transforms
  const updateStyles = useCallback(
    (
      ...
      if (!isMobile) {
        // Desktop: Mouse-based transforms
        element.style.transform = `
          translate3d(${cursorX}px, ${cursorY}px, 0) 
          scale(${scale})
        `;

        // Use CSS filters to simulate your gradient intensity changes
        const brightness = 0.8 + (leftIntensity + rightIntensity) * 0.4;
        const contrast = 1 + (leftIntensity - rightIntensity) * 0.2;
        const hueRotate = (leftIntensity - rightIntensity) * 15;

        element.style.filter = `
          brightness(${brightness}) 
          contrast(${contrast}) 
          hue-rotate(${hueRotate}deg)
          saturate(${1 + Math.abs(leftIntensity - rightIntensity) * 0.3})
        `;

        element.style.opacity = `${0.6 + (leftIntensity + rightIntensity) * 0.2}`;
      } else {
        // Mobile: Scroll-based transforms
        element.style.transform = `
          scale(${mobileScale}) 
        ...
        element.style.filter = `
          brightness(${0.7 + mobileIntensity * 0.5})
          saturate(${1 + mobileIntensity * 0.4})
        `;

        element.style.opacity = `${0.5 + mobileIntensity * 0.3}`;
      }
    },
    [isMobile]
  );

  // Your exact mouse move logic - unchanged
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      if (isMobile || !containerRef.current) return;
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        const scale = 1 + (leftIntensity + rightIntensity) * 0.02;

        updateStyles(leftIntensity, rightIntensity, cursorX, cursorY, scale);
  ...
  // Your exact scroll logic - unchanged
  const handleScroll = useCallback(() => {
    if (!isMobile) return;
  ...
  // Your exact event setup - unchanged
  useEffect(() => {
    handleResize();
    window.addEventListener("resize", handleResize, { passive: true });
    return () => window.removeEventListener("resize", handleResize);
  ...
  return (
    <div
      ...
      ref={containerRef}
    >
      {/* Single PNG with your exact same control approach */}
      <div
        ref={imageRef}
        className="absolute inset-0 w-full h-full"
        style={{
          backgroundImage: `url(${AnimatedGradient})`,
